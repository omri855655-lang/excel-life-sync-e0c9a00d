...
import { Plus, Trash2, Download, Check, Clock, AlertCircle } from "lucide-react";
import { cn } from "@/lib/utils";
import { Task, taskHeaders } from "@/data/initialTasks";
import {
  Select,
  SelectContent,
  SelectItem,
...
const TaskSpreadsheet = ({ initialTasks, title, readOnly = false }: TaskSpreadsheetProps) => {
  ...
  const handleCellChange = useCallback(
    (taskId: number, field: keyof Task, value: string) => {
      setTasks((prev) =>
        prev.map((task) =>
  ...
  const deleteTask = () => {
    if (selectedRow !== null && tasks.length > 0) {
      setTasks((prev) => prev.filter((_, i) => i !== selectedRow));
      setSelectedRow(null);
    }
  };
