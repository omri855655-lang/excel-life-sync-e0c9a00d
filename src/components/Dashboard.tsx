...
import { BookOpen, Tv, CheckCircle, Clock, Eye, TrendingUp, LayoutGrid, CalendarDays, Settings2, ChevronUp, ChevronDown, EyeOff, RotateCcw } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const renderSection = (sectionId: string) => {
    ...
    switch (sectionId) {
      case "date":
        return (
          <Card key="date" className="bg-gradient-to-br from-primary/5 to-accent/5 border-primary/20">
      ...
      case "quickStats":
        if (vm === "compact") {
          return (
            <div key="quickStats" className="flex gap-4 flex-wrap">
              <span className="text-sm"><BookOpen className="h-4 w-4 inline mr-1" />{stats.totalBooks} ספרים ({stats.booksRead} נקראו)</span>
              <span className="text-sm"><Tv className="h-4 w-4 inline mr-1" />{stats.totalShows} סדרות ({stats.showsWatched} נצפו)</span>
            </div>
          );
        }
        if (vm === "cards") {
          return (
            <div key="quickStats" className="grid grid-cols-2 gap-3">
              ...
              <Card className="bg-accent/5 border-accent/20 p-3 text-center">
                ...
                <div className="text-xs text-muted-foreground">נקראו</div>
              </Card>
              <Card className="bg-primary/5 border-primary/20 p-3 text-center">
                <Tv className="h-5 w-5 mx-auto text-primary mb-1" />
                <div className="text-xl font-bold">{stats.totalShows}</div>
                <div className="text-xs text-muted-foreground">סדרות/סרטים</div>
              </Card>
              <Card className="bg-accent/5 border-accent/20 p-3 text-center">
                ...
                <div className="text-xs text-muted-foreground">נצפו</div>
              </Card>
            </div>
          );
        }
        return (
          <div key="quickStats" className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><BookOpen className="h-4 w-4" />סה"כ ספרים</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{stats.totalBooks}</div></CardContent></Card>
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><CheckCircle className="h-4 w-4 text-accent-foreground" />ספרים שנקראו</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-accent-foreground">{stats.booksRead}</div></CardContent></Card>
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Tv className="h-4 w-4" />סה"כ סדרות/סרטים</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{stats.totalShows}</div></CardContent></Card>
            <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2"><Eye className="h-4 w-4 text-accent-foreground" />נצפו</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-accent-foreground">{stats.showsWatched}</div></CardContent></Card>
          </div>
        );
      ...
      case "booksChart":
        return (
          <Card key="booksChart">
            <CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />התפלגות ספרים</CardTitle></CardHeader>
            <CardContent>
              {booksChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height={vm === "compact" ? 120 : 200}>
                  <PieChart>
                    <Pie data={booksChartData} cx="50%" cy="50%" innerRadius={vm === "compact" ? 20 : 40} outerRadius={vm === "compact" ? 50 : 80} paddingAngle={5} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                      {booksChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
              ) : <div className="h-[120px] flex items-center justify-center text-muted-foreground">אין ספרים עדיין</div>}
            </CardContent>
          </Card>
        );
      ...
      case "showsChart":
        return (
          <Card key="showsChart">
            <CardHeader><CardTitle className="flex items-center gap-2"><Tv className="h-5 w-5" />התפלגות סדרות/סרטים</CardTitle></CardHeader>
            <CardContent>
              {showsChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height={vm === "compact" ? 120 : 200}>
                  <PieChart>
                    <Pie data={showsChartData} cx="50%" cy="50%" innerRadius={vm === "compact" ? 20 : 40} outerRadius={vm === "compact" ? 50 : 80} paddingAngle={5} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                      {showsChartData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
              ) : <div className="h-[120px] flex items-center justify-center text-muted-foreground">אין סדרות עדיין</div>}
            </CardContent>
          </Card>
        );
      ...
      case "comparison":
        return (
          <Card key="comparison">
            <CardHeader><CardTitle>השוואה כללית</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={vm === "compact" ? 120 : 200}>
                <BarChart data={comparisonData} layout="vertical">
                  <XAxis type="number" /><YAxis dataKey="name" type="category" width={100} />
                  <Tooltip /><Legend />
                  <Bar dataKey="total" name="סה״כ" fill="hsl(var(--primary))" />
                  <Bar dataKey="completed" name="הושלמו" fill="hsl(var(--accent))" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
      ...
      case "progress":
        return (
          <div key="progress" className="grid md:grid-cols-2 gap-4">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm">התקדמות קריאה</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm"><span>בקריאה</span><span className="font-medium">{stats.booksReading}</span></div>
                  ...
                  <div className="flex justify-between text-sm"><span>ממתינים</span><span className="font-medium">{stats.booksToRead}</span></div>
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm">התקדמות צפייה</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm"><span>בצפייה</span><span className="font-medium">{stats.showsWatching}</span></div>
                  ...
                  <div className="flex justify-between text-sm"><span>ממתינים</span><span className="font-medium">{stats.showsToWatch}</span></div>
                ...
                <Card key={board.id} className="mb-4">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div><div className="text-sm text-muted-foreground">סה"כ</div><div className="text-2xl font-bold">{bs.total}</div></div>
          ...
          <Card className="mb-4 border-primary/30">
            <CardHeader className="py-3">
              <CardTitle className="text-sm flex items-center justify-between">
                <span>התאמה אישית של הדשבורד</span>
                <Button variant="ghost" size="sm" className="h-7 gap-1 text-xs" onClick={resetToDefault}>
                  <RotateCcw className="h-3 w-3" />
                  איפוס
                </Button>
              </CardTitle>
              <p className="text-xs text-muted-foreground">הסתר/הצג בלוקים, שנה סדר ובחר סוג תצוגה</p>
            </CardHeader>
            <CardContent className="space-y-1 pt-0">
              {sections.map((section, idx) => (
                <div key={section.id} className={cn(
                  "flex items-center gap-2 p-2 rounded-md border text-sm",
                  section.visible ? "bg-card border-border" : "bg-muted/30 border-border/50 opacity-60"
                )}>
